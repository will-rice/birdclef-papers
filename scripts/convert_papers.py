...
from scripts._convert.output import paper_slug  # noqa: E402
...
def needs_conversion(row: PaperRow, papers_dir: Path, *, force: bool = False) -> bool:
    ...
    if force:
        return True
    target = papers_dir / row.year / f"{paper_slug(row.arxiv_id)}.md"
    if not target.exists():
        return True
    # Non-arXiv sources (ss:, biorxiv:, dblp:, pwc:) have no HTML tier to gain,
    # so convert them once and never retry.
    if ":" in row.arxiv_id:
        return False
    try:
...
def _process_paper(row: PaperRow, force: bool = False) -> None:
    ...
    # Only bare arXiv ids are convertible from source; prefixed ids (ss:, biorxiv:,
    # dblp:, pwc:) are handled as PDF (Semantic Scholar) or metadata-only.
    is_arxiv = ":" not in row.arxiv_id
    body = ""
    bbl_text = ""
    bib_text = ""
    converter = "none"
    ...
    record = output.PaperRecord(
        ...
        github_repo=sources.fetch_hf_github_repo(row.arxiv_id),
    )
...
def _build_corpus_index() -> dict[str, str]:
    ...
    # Only bare arXiv ids are citable as local siblings via their arXiv id.
    return {r.arxiv_id: r.year for r in rows if ":" not in r.arxiv_id}
...
def _run_remediation_pass(rows: list[PaperRow]) -> None:
    ...
    for row in rows:
        md_path = PAPERS_DIR / row.year / f"{paper_slug(row.arxiv_id)}.md"
    ...
    for row, md_path, front, body in candidates:
        ...
        record = PaperRecord(
            ...
            github_repo=front.get("github_repo", ""),
...
def _regenerate_indexes(rows: list[PaperRow]) -> None:
    ...
    all_entries = [
        ...
        for r in rows
        if (PAPERS_DIR / r.year / f"{paper_slug(r.arxiv_id)}.md").exists()
    ]
