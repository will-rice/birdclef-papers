...
def fetch_arxiv_html(arxiv_id: str) -> HtmlPage | None:
    ...
    """
    # Only bare arXiv ids have an HTML rendering; prefixed ids (ss:, biorxiv:,
    # dblp:, pwc:) never do.
    if ":" in arxiv_id:
        return None
...
def fetch_hf_github_repo(arxiv_id: str) -> str:
    """Return the GitHub repo URL linked on the paper's Hugging Face page, or ""."""
    # Hugging Face indexes papers by bare arXiv id only.
    if ":" in arxiv_id:
        return ""
    ...
    try:
        ...
        logging.info("No Hugging Face paper page for %s: %s", arxiv_id, exc)
        return ""
    return (data.get("githubRepo") or "").strip()
