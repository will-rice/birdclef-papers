...
def test_fetch_hf_github_repo_found(monkeypatch) -> None:
    def fake_urlopen(req, timeout=0):  # noqa: ARG001
        assert "huggingface.co/api/papers/2606.31145" in req.full_url
        return _FakeResponse(b'{"githubRepo": "https://github.com/AmirAbaskohi/SeKV"}')

    monkeypatch.setattr("urllib.request.urlopen", fake_urlopen)
    assert fetch_hf_github_repo("2606.31145") == "https://github.com/AmirAbaskohi/SeKV"


def test_fetch_hf_github_repo_absent_field(monkeypatch) -> None:
    def fake_urlopen(req, timeout=0):  # noqa: ARG001
        return _FakeResponse(b'{"id": "1211.3711", "title": "RNN-T"}')

    monkeypatch.setattr("urllib.request.urlopen", fake_urlopen)
    assert fetch_hf_github_repo("1211.3711") == ""


def test_fetch_hf_github_repo_404(monkeypatch) -> None:
    def fake_urlopen(req, timeout=0):  # noqa: ARG001
        raise _http_404(req.full_url)

    monkeypatch.setattr("urllib.request.urlopen", fake_urlopen)
    assert fetch_hf_github_repo("0000.00000") == ""


def test_fetch_hf_github_repo_skips_non_arxiv(monkeypatch) -> None:
    def fake_urlopen(req, timeout=0):  # noqa: ARG001
        raise AssertionError("network should not be called for non-arxiv IDs")

    monkeypatch.setattr("urllib.request.urlopen", fake_urlopen)
    assert fetch_hf_github_repo("s2:abc") == ""
